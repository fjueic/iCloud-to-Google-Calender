...
var file = folder.getFilesByName("data.json").next();
var jsonString = file.getBlob().getDataAsString();
var data = JSON.parse(jsonString);
var sheet = SpreadsheetApp.openById(data["id"]);
...
function test(){
  var events = calendar.getEvents(new Date(`${"3/30/2023"} ${"9:35"}`), 
      new Date(`${"3/30/2023"} ${"10:25"}`));
  Logger.log(events)
